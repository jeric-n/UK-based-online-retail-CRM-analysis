...
# ============================================================
# 5. REFINED MODEL (Addressing Multicollinearity)
# ============================================================
cat("\n========================================\n")
cat("REFINED MODEL (After VIF Analysis)\n")
cat("========================================\n")

# Remove TotalItems due to high correlation with TotalOrders
# Keep more interpretable variables
model_refined <- lm(TotalRevenue ~ TotalOrders + TotalProducts + 
                      AvgItemPrice + DaysSinceFirst + IsUK, 
                    data = customer_data)
print(summary(model_refined))

# VIF for refined model
cat("\n--- VIF Analysis (Refined Model) ---\n")
vif_refined <- vif(model_refined)
print(vif_refined)

# ============================================================
# 6. STEPWISE REGRESSION (AIC-based model selection)
# ============================================================
cat("\n========================================\n")
cat("STEPWISE REGRESSION (AIC)\n")
cat("========================================\n")
...
cat("\nFinal Model Variables:", paste(names(coef(model_step))[-1], collapse = ", "), "\n")
cat("AIC:", AIC(model_step), "\n")

# VIF for stepwise model
cat("\n--- VIF Analysis (Stepwise Model) ---\n")
...
coef_data <- data.frame(
  Variable = names(coef(model_refined))[-1],
  Coefficient = coef(model_refined)[-1],
  StdError = summary(model_refined)$coefficients[-1, 2]
)
...
ggplot(coef_data, aes(x = reorder(Variable, Coefficient), y = Coefficient)) +
  ...
  coord_flip() +
  labs(title = "Multiple Regression Coefficients (95% CI)",
...
vif_df <- data.frame(Variable = names(vif_refined), VIF = as.numeric(vif_refined))
...
customer_data$PredictedRevenue <- predict(model_refined, customer_data)
...
legend("topleft", 
       legend = c(paste("R² =", round(summary(model_refined)$r.squared, 3)),
...
cat("\n2. MULTIPLE REGRESSION (Refined Model):\n")
cat("   R-squared:", round(summary(model_refined)$r.squared, 4), "\n")
cat("   Adj. R-squared:", round(summary(model_refined)$adj.r.squared, 4), "\n")
cat("   Significant predictors (p < 0.05):\n")
signif_vars <- summary(model_refined)$coefficients[, 4] < 0.05
print(names(signif_vars[signif_vars == TRUE]))

cat("\n3. VIF ANALYSIS:\n")
cat("   All variables below threshold: ", all(vif_refined < 5), "\n")
if (any(vif_refined > 5)) {
  cat("   Variables with VIF > 5:", names(vif_refined[vif_refined > 5]), "\n")
}

cat("\n4. MODEL EQUATION:\n")
cat("   TotalRevenue = ", round(coef(model_refined)[1], 2))
for (i in 2:length(coef(model_refined))) {
  sign <- ifelse(coef(model_refined)[i] >= 0, " + ", " - ")
  cat(sign, round(abs(coef(model_refined)[i]), 2), "*", names(coef(model_refined))[i])
}
cat("\n")
...
regression_results <- list(
  ...
  refined_model = model_refined,
  stepwise_model = model_step,
  vif_full = vif_values,
  vif_refined = vif_refined
)
